...
def sample_binary_arrays_large_N(N, K, enforce_symmetry=False):
    sampled = set()
    result = []
    while len(result) < K:
        arr = np.random.choice([-1, 1], size=N)
        if enforce_symmetry and arr[0] == -1:
            arr = -arr
        arr_tuple = tuple(arr)
...
# Ensures that the first element of the control input is positive
...
for h in h_vals:
    ...
    rand_u_vec = (
        sample_binary_arrays_large_N(
            m * (h + 1), samples, enforce_symmetry=enforce_symmetry
        )
        * eta
    )
